...
- `routes.ts`: REST routes for `tasks`, `images`, and `canvas-images`. Includes `GET /ping`, CRUD-like endpoints, idempotent image upsert, and `?full=true` image listing.
- `storage.ts`: SQLite persistence using `better-sqlite3`. Creates `tasks`, `images`, and `canvas_images` tables with JSON blobs and created-at indexes. Migrates legacy JSON files when SQLite tables are empty.
...
4. CRUD methods serialize domain objects as JSON in table rows keyed by `id`.
