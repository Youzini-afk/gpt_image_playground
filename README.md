...
- `API_PROXY_URL`：配置内置代理实际转发到的完整 API 基础地址（仅开启代理或前端未传目标时使用）。代理不会自动补 `/v1`，OpenAI 兼容接口通常必须填写到版本前缀，如 `https://api.openai.com/v1`。
- `ENABLE_API_PROXY`：设为 `true` 开启容器内置同源代理，用于解决浏览器跨域（CORS）限制。开启后，前端 **API 代理** 开关默认开启，浏览器会请求同源的 `/api-proxy/{接口相对路径}`，再由服务端拼接到页面设置中的 API URL 或 `API_PROXY_URL` 后转发；用户仍可在设置中手动关闭。
