...
import { useStore, getCachedImage, ensureImageCached, retainImageDisplayUrl, releaseImageDisplayUrl, reuseConfig, editOutputs, removeTask, showCodexCliPrompt, getCodexCliPromptKey, retryTask, addImageToCanvas } from '../store'
...
export default function DetailModal() {
  ...
  useEffect(() => {
    ...
    const retainedOutputImageIds: string[] = []
    for (const imageId of outputImageIds) {
      const cached = getCachedImage(imageId)
      if (cached) {
        setOutputImage(imageId, cached)
      }
      retainedOutputImageIds.push(imageId)
      retainImageDisplayUrl(imageId)
        .then((displayUrl) => {
          if (displayUrl) setOutputImage(imageId, displayUrl)
        })
        .catch(() => {})
    }
    ...
    return () => {
      cancelled = true
      retainedOutputImageIds.forEach(releaseImageDisplayUrl)
    }
  }, [task?.outputImages])
