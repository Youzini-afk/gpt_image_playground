...
import { isApiProxyAvailable, isApiProxyForced, isApiProxyLocked, readClientDevProxyConfig } from '../lib/devProxy'
import { testServerStorage } from '../lib/storage'
import { useStore, exportData, importData, clearData, switchStorageMode, type SettingsTab } from '../store'
import {
...
export default function SettingsModal() {
  ...
  const apiProxyForced = isApiProxyForced(apiProxyConfig)
  ...
  const apiProxyChecked = activeProfileApiProxyEligible && (apiProxyForced || apiProxyLocked || activeProfile.apiProxy)
    ...
    const nextDraft = normalizeSettings({
      ...
      profiles: displaySettings.profiles.map((profile) => ({
        ...
        apiProxy: isProfileApiProxyEligible(displaySettings, profile) && apiProxyAvailable
          ? (apiProxyForced || apiProxyLocked || profile.apiProxy)
          : false,
      })),
    })
    setDraft(nextDraft)
  ...
  }, [apiProxyAvailable, apiProxyForced, apiProxyLocked, showSettings, settings, reusedTaskApiProfileId])
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      const nextApiProxy = isProfileApiProxyEligible(nextDraft, profile) && apiProxyAvailable ? (apiProxyForced || apiProxyLocked || profile.apiProxy) : false
            ...
            {activeTab === 'api' && (
                ...
                <label className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                      {apiProxyEnabled ? (
                        <span className="text-blue-600 dark:text-blue-400">已开启代理，请求会先到同源 /api-proxy，后端会转发到此处填写的 API URL。</span>
              ...
              {apiProxyAvailable && activeProviderIsOpenAICompatible && !activeCustomProviderAsync && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">API 代理</span>
                      <button
                        type="button"
                        onClick={() => {
                          if (!apiProxyForced && !apiProxyLocked) updateActiveProfile({ apiProxy: !activeProfile.apiProxy }, true)
                        }}
                        disabled={apiProxyForced || apiProxyLocked}
                        className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${apiProxyChecked ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'} ${apiProxyForced || apiProxyLocked ? 'cursor-not-allowed opacity-70' : ''}`}
                      role="switch"
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyForced
                      ? '当前 Docker/Zeabur 部署已强制使用同源代理；请求经服务器转发到上游 API，上方 API URL 仍会作为后端转发目标。'
                      : apiProxyLocked
                        ? '当前部署已锁定 API 代理为开启；请求经服务器转发到上游 API。'
                        : '当前部署提供同源代理时默认开启，可手动关闭。开启后请求经服务器转发到上游 API，可绕过浏览器跨域限制。'}
                  </div>
                </div>
              )}
              ...
              {activeProfile.provider === 'openai' && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">编辑图字段</span>
                    <button
                      type="button"
                      onClick={() => updateActiveProfile({ editImageField: activeProfile.editImageField === 'image' ? 'image[]' : 'image' }, true)}
                      className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${activeProfile.editImageField === 'image' ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                      role="switch"
                      aria-checked={activeProfile.editImageField === 'image'}
                      aria-label="编辑图字段"
                    >
                      <span className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${activeProfile.editImageField === 'image' ? 'translate-x-[14px]' : 'translate-x-[2px]'}`} />
                    </button>
                  </div>
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {activeProfile.editImageField === 'image'
                      ? '兼容只识别 image 字段的编辑接口。'
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-3 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-sm font-bold text-gray-800 dark:text-gray-100">数据存储</h4>
                  </div>
                  <Select
                  ...
                  {draft.storageMode === 'server' && testingConnection && (
                    <div className="rounded-xl bg-blue-50 px-3 py-2 text-xs text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">
                      正在检测服务端存储连接...
                    </div>
                  )}
                  {draft.storageMode === 'server' && connectionResult && !testingConnection && (
                    <div className={`rounded-xl px-3 py-2 text-xs ${connectionResult.ok ? 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-300' : 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-300'}`}>
                      {connectionResult.ok ? '服务端存储连接正常' : `服务端存储不可用：${connectionResult.error || '未知错误'}`}
                    </div>
                  )}
                </div>
