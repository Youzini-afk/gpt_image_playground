...
async function putTask(task: TaskRecord) {
  await getStorage().putTask(getPersistableTask(task))
}
...
async function dbClearTasks() {
  await getStorage().clearTasks()
}

async function getImage(id: string) {
  return getStorage().getImage(id)
}
...
async function putImage(image: StoredImage) {
  await getStorage().putImage(image)
  persistedImageIds.add(image.id)
}
...
export function ensureImageDisplayUrl(id: string): Promise<string | undefined> {
  return ensureImageDisplayUrlCached(id)
}

export function clearImageDisplayUrlCache() {
...
export function getPersistedState(state: AppState) {
  ...
  const persistedInputImages = galleryInputDraft?.inputImages ?? (state.appMode === 'gallery' ? state.inputImages : [])
  return {
    settings,
    params: state.params,
    ...(settings.persistInputOnRestart && (state.appMode === 'gallery' || galleryInputDraft)
      ? {
          ...
          inputImages: persistedInputImages.map((img) => ({ id: img.id, dataUrl: '' })),
          inputImageIds: persistedInputImages.map((img) => img.id),
        }
      : {}),
...
async function replaceStoredAgentConversations(conversations: AgentConversation[]) {
  await getStorage().replaceAgentConversations(conversations.map(getPersistableAgentConversation))
}
...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  const galleryInputDraft = settings.persistInputOnRestart
    ? normalizeAgentInputDraft(persisted.galleryInputDraft ?? {
        ...
        inputImages: Array.isArray(persisted.inputImages)
          ? persisted.inputImages
          : Array.isArray(persisted.inputImageIds)
            ? persisted.inputImageIds.map((id) => ({ id, dataUrl: '' }))
            : [],
        maskDraft: null,
  ...
  return {
    ...
    inputImageIds: (restoredAgentDraft ? restoredAgentDraft.inputImages : galleryInputDraft?.inputImages ?? []).map((img) => img.id),
  }
}
    ...
    (set, get) => ({
      ...
      setSettings: (s) => set((st) => {
        ...
        const hasLegacyOverrides =
          ...
          incoming.editImageField !== undefined ||
          ...
          merged.profiles = merged.profiles.map((profile) =>
            profile.id === merged.activeProfileId
              ? {
                  ...
                  editImageField:
                    incoming.editImageField === 'image' || incoming.editImageField === 'image[]'
                      ? incoming.editImageField
                      : profile.editImageField,
      ...
      addInputImage: (img) =>
        set((s) => {
          ...
          return {
            ...syncActiveInputDraft(s, { inputImages }),
      ...
      moveInputImage: (fromIdx, toIdx) =>
        set((s) => {
          ...
          return {
            ...
            inputImageIds: images.map((image) => image.id),
          }
        }),
      maskDraft: null,
...
export async function initStorageMode() {
  ...
  if (settings.storageMode === 'server') {
    ...
    if (result.ok) {
      ...
      useStore.getState().setSettings({ storageMode: 'local' })
      useStore.getState().showToast(`服务器存储不可用，已切换到本地存储${result.error ? `：${result.error}` : ''}`, 'error')
    }
  } else {
    setStorageMode('local')
  }
...
function createSettingsForApiProfile(settings: AppSettings, profile: ApiProfile): AppSettings {
  ...
  return normalizeSettings({
    ...
    editImageField: profile.editImageField,
...
export async function initStore() {
  ...
  const storedAgentConversations = normalizeAgentConversations(await getStorage().getAllAgentConversations())
  ...
  for (const canvasImage of canvasImages) referencedIds.add(canvasImage.imageId)
...
export async function addImageToCanvas(dataUrl: string, label?: string): Promise<string> {
  ...
  return canvasImage.id
}

/** 从画布工作台移除图片 */
export async function removeCanvasImage(canvasImage: CanvasImage) {
  const { canvasImages, tasks, inputImages, galleryInputDraft } = useStore.getState()
  const remaining = canvasImages.filter((item) => item.id !== canvasImage.id)
  ...
  for (const item of remaining) stillUsed.add(item.imageId)
  for (const image of inputImages) stillUsed.add(image.id)
  if (!stillUsed.has(canvasImage.imageId)) {
    await deleteImage(canvasImage.imageId)
...
export async function addCanvasImageToInput(canvasImage: CanvasImage) {
  ...
  if (inputImages.find((image) => image.id === canvasImage.imageId)) {
...
async function deleteUnreferencedImageIds(imageIds: Iterable<string>) {
  ...
  const { tasks, inputImages, galleryInputDraft, canvasImages } = useStore.getState()
  const stillUsed = new Set<string>()
  for (const task of tasks) addTaskReferencedImageIds(stillUsed, task)
  addAgentReferencedImageIds(stillUsed)
  addInputDraftReferencedImageIds(stillUsed, galleryInputDraft)
  for (const img of inputImages) stillUsed.add(img.id)
  for (const canvasImage of canvasImages) stillUsed.add(canvasImage.imageId)
...
async function executeAgentRound(
  ...
  try {
    ...
    const resolveReferenceImages = async (referenceIds: string[]): Promise<{ dataUrls: string[]; imageIds: string[] }> => {
      ...
      const addResolvedImage = async (imageId?: string | null) => {
        if (!imageId || imageIds.includes(imageId)) return
        const dataUrl = await ensureImageCached(imageId)
        if (!dataUrl) return
      ...
      for (const refId of referenceIds) {
        ...
        for (const r of getAgentRoundPath(latestConv, roundId)) {
          for (let inputIdx = 0; inputIdx < r.inputImageIds.length; inputIdx++) {
            if (getAgentCurrentReferenceId(r, inputIdx) === refId) {
              await addResolvedImage(r.inputImageIds[inputIdx])
            }
          }
          ...
          for (let imgIdx = 0; imgIdx < outputImages.length; imgIdx++) {
            ...
            if (generatedRefId === refId) {
              await addResolvedImage(outputImages[imgIdx])
            }
          }
        }
      }
    ...
    const markAgentImageTaskError = async (toolCallId: string, error: string) => {
    ...
    const executeBatchFunctionCall = async (functionCallItem: ResponsesOutputItem): Promise<string> => {
      ...
      const batchPromises = batchExecutionItems.map(async ({ item, batchToolCallId, references, referenceIds }) => {
        try {
          ...
          } else if (!batchResult.image) {
            await markAgentImageTaskError(batchToolCallId, batchResult.error || '图片生成失败')
          }

          return batchResult
        } catch (error) {
          await markAgentImageTaskError(
            batchToolCallId,
...
export async function removeMultipleTasks(taskIds: string[]) {
  const { tasks, setTasks, inputImages, galleryInputDraft, canvasImages, showToast, selectedTaskIds } = useStore.getState()
...
export async function removeTask(task: TaskRecord) {
  const { tasks, setTasks, inputImages, galleryInputDraft, canvasImages, showToast } = useStore.getState()
...
export async function clearData(options: ClearOptions = { clearConfig: true, clearTasks: true }) {
  ...
  if (options.clearTasks) {
    await dbClearTasks()
    await getStorage().clearAgentConversations()
    await clearImages()
    await getStorage().clearCanvasImages()
    imageCache.clear()
    imageLoadPromises.clear()
    persistedImageIds.clear()
    thumbnailCache.clear()
...
/** 清空所有数据（含配置重置），保留旧组件/测试调用入口。 */
export async function clearAllData() {
  await clearData({ clearConfig: true, clearTasks: true })
}
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    const canvasImages = options.exportTasks ? await getStorage().getAllCanvasImages() : []
    ...
    for (const canvasImage of canvasImages) {
      const prev = imageCreatedAtFallback.get(canvasImage.imageId)
      if (prev == null || canvasImage.createdAt < prev) {
        imageCreatedAtFallback.set(canvasImage.imageId, canvasImage.createdAt)
      }
    }
    ...
    if (options.exportTasks) {
      ...
      manifest.canvasImages = canvasImages
...
export async function importData(file: File, options: ImportOptions = { importConfig: true, importTasks: true }): Promise<boolean> {
  try {
    ...
    if (options.importTasks && data.tasks && data.imageFiles) {
      ...
      if (data.canvasImages?.length) {
        for (const canvasImage of data.canvasImages) {
          await getStorage().putCanvasImage(canvasImage)
        }
        useStore.getState().setCanvasImages(await getStorage().getAllCanvasImages())
      }
