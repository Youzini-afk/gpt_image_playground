...
export const DEFAULT_EDIT_IMAGE_FIELD: EditImageField = 'image[]'
...
export function createDefaultOpenAIProfile(overrides: Partial<ApiProfile> = {}): ApiProfile {
  ...
  return {
    ...
    editImageField: DEFAULT_EDIT_IMAGE_FIELD,
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  if (provider === 'fal') {
    return {
      ...
      codexCli: false,
      apiProxy: false,
      editImageField: savedDraft?.editImageField ?? DEFAULT_EDIT_IMAGE_FIELD,
  ...
  return {
    ...
    editImageField: savedDraft?.editImageField ?? profile.editImageField ?? DEFAULT_EDIT_IMAGE_FIELD,
    responseFormatB64Json: savedDraft?.responseFormatB64Json,
...
function normalizeProviderDraft(input: unknown, provider: ApiProvider, customProviderIds: Set<string>): ApiProfileProviderDraft {
  ...
  return {
    ...
    editImageField: input.editImageField === 'image' ? 'image' : input.editImageField === 'image[]' ? 'image[]' : fallback.editImageField,
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  const editImageField: EditImageField = record.editImageField === 'image' || record.editImageField === 'image[]'
    ? record.editImageField
    : defaults.editImageField
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const legacyProfile = createDefaultOpenAIProfile({
    ...
    editImageField: record.editImageField === 'image' ? 'image' : DEFAULT_EDIT_IMAGE_FIELD,
    responseFormatB64Json: record.responseFormatB64Json === true ? true : undefined,
  ...
  return {
    ...
    editImageField: active.editImageField,
    storageMode: record.storageMode === 'server' ? 'server' : DEFAULT_STORAGE_MODE,
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  return {
    ...
    editImageField: record.editImageField === 'image' || record.editImageField === 'image[]' ? record.editImageField : profile.editImageField,
...
function isDefaultOpenAIProfile(profile: ApiProfile): boolean {
  return profile.id === DEFAULT_OPENAI_PROFILE_ID &&
    ...
    profile.editImageField === DEFAULT_EDIT_IMAGE_FIELD &&
