...
const storageMock = vi.hoisted(() => {
  ...
  const adapter = {
    ...
    replaceAgentConversations: vi.fn(async (conversations: AgentConversation[]) => {
      agentConversations.clear()
      for (const conversation of conversations) agentConversations.set(conversation.id, conversation)
    }),
    clearAgentConversations: vi.fn(async () => { agentConversations.clear() }),
  }
  ...
  return {
    ...
    deleteAgentConversation: vi.fn(async (id: string) => { agentConversations.delete(id) }),
    clearAgentConversations: vi.fn(async () => { agentConversations.clear() }),
    replaceAgentConversations: vi.fn(async (conversations: AgentConversation[]) => {
      agentConversations.clear()
      for (const conversation of conversations) agentConversations.set(conversation.id, conversation)
...
vi.mock('./lib/db', () => ({
  ...
  getAllAgentConversations: storageMock.getAllAgentConversations,
  putAgentConversation: storageMock.putAgentConversation,
  deleteAgentConversation: storageMock.deleteAgentConversation,
  clearAgentConversations: storageMock.clearAgentConversations,
  replaceAgentConversations: storageMock.replaceAgentConversations,
}))
...
import { cleanStaleAgentInputDrafts, clearFailedTasks, deleteAgentRoundFromConversation, deleteFavoriteCollection, editOutputs, ensureImageCached, ensureImageDisplayUrlCached, ensureImageThumbnailCached, getActiveAgentRounds, getCachedImageDisplayUrl, getErrorToastMessage, getPersistedState, getTaskApiProfile, importData, initStore, markInterruptedOpenAIRunningTasks, migratePersistedState, regenerateAgentAssistantMessage, releaseImageDisplayUrl, remapAgentRoundMentionsForPathChange, removeTask, retainImageDisplayUrl, reuseConfig, submitAgentMessage, submitTask, useStore } from './store'
...
describe('mask draft lifecycle in store actions', () => {
  beforeEach(() => {
    ...
    storageMock.agentConversations.clear()
    storageMock.getAllAgentConversations.mockImplementation(async () => [...storageMock.agentConversations.values()])
    storageMock.putAgentConversation.mockImplementation(async (conversation: AgentConversation) => {
      storageMock.agentConversations.set(conversation.id, conversation)
      return conversation.id
    })
    storageMock.clearAgentConversations.mockImplementation(async () => { storageMock.agentConversations.clear() })
    storageMock.replaceAgentConversations.mockImplementation(async (conversations: AgentConversation[]) => {
      storageMock.agentConversations.clear()
      for (const conversation of conversations) storageMock.agentConversations.set(conversation.id, conversation)
    })
  })
  ...
  it('falls back to the original output when transparent post-processing fails', async () => {
    ...
    vi.mocked(callImageApi).mockResolvedValueOnce({
      images: ['data:image/png;base64,generated-fallback'],
      actualParams: { output_format: 'png' },
      actualParamsList: [{ output_format: 'png' }],
      revisedPrompts: [],
    })
    ...
    const outputImage = await getImage(task.outputImages[0])
    expect(outputImage?.dataUrl).toBe('data:image/png;base64,generated-fallback')
    warnSpy.mockRestore()
    await clearTasks()
    await clearImages()
  })
...
describe('agent conversation persistence', () => {
  beforeEach(async () => {
    ...
    useStore.setState({ agentConversationsLoaded: false })
  })
